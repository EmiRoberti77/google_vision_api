...
def draw_boundary_normalized(image_file, vertices, caption=''):
    ...
    pil_image = Image.open(image_file)
    draw = ImageDraw.Draw(pil_image)
    xys = [(vertex.x * pil_image.size[0], vertex.y * pil_image.size[1])
...
class VisionAI:
    ...
    Logo_Detection = namedtuple(
        'Logo_Detection', ('description', 'score', 'bounding_poly'))
    Object_Detection = namedtuple(
        'Object_Detection', ('name', 'score', 'bounding_poly'))
    Landmark_Detection = namedtuple(
        'Landmark_Detection', ('description', 'score', 'bounding_poly', 'location'))
    ...
    Web_Detection = namedtuple('Web_Detection', ('web_entities', 'full_matching_images',
                               'visually_similar_images', 'pages_with_matching_images', 'best_guess_labels'))
    Web_Entity = namedtuple('Web_Entity', ('description', 'score'))
    Page_Matching_Image = namedtuple('Page_Matching_Image', (
        'url', 'page_title', 'full_matching_image_urls', 'partial_matching_image_urls'))
    ...
    Text_Detection = namedtuple(
        'Text_Detection', ('description', 'bounding_poly'))
    ...
    def text_detection(self):
        ...
        if texts:
            results = []
            for text in texts:
                results.append(self.Text_Detection(
                    text.description, text.bounding_poly.vertices))
            return results
        return
    ...
    def web_detection(self):
        ...
        if web_detection:
            return self.Web_Detection(
                [self.Web_Entity(web_entity.description, float('{0:.2f}'.format(
                    web_entity.score))) for web_entity in web_detection.web_entities],
                [url.url for url in web_detection.full_matching_images],
                [url.url for url in web_detection.visually_similar_images],
                [self.Page_Matching_Image(page.url, page.page_title, [url.url for url in page.full_matching_images], [
                                          url.url for url in page.partial_matching_images]) for page in web_detection.pages_with_matching_images],
